...
The BaseInfo summary row MUST mirror the Figma layout and copy: greeting text, level badge, buyer ID, GIGA Index value, Beans count, and the "招商经理" entry in the same order as node 833-27929.

#### Scenario: Summary Rendering
- **WHEN** BaseInfo renders
- **THEN** the left side shows "Hi, {name}" plus the level badge
- **AND** the right side lists the ID, GIGA Index, Beans, and manager entry with the same dividers as the design.

### Requirement: Metric Link Hover
The GIGA Index and Beans text SHALL act as inline metric links that only add an underline on hover (no glow or shadow), matching node 833-28679.

#### Scenario: Hover Feedback
- **WHEN** a user hovers the GIGA Index or Beans text
